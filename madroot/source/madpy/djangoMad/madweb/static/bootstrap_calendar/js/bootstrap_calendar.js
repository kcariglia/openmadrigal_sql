    ...
    this.each(function(){
        ...
        function showCalendar(){
            ...
            */
            ...
            var lblDateControl = $('<td colspan=5 class="year"></td>');
        ...
        function showDaysOfMonth(month, year){
        	var HtmlSelectMonths='<select ">';
        	for (i=0;i<=selectedMonths.length;i++){
        		if (selectedMonths[i] != undefined ){
        			if (selectedMonthsNumbers[i].charAt(0)=="0"){
                        month_number = selectedMonthsNumbers[i].substring(1);
                    }
        			month_number = parseInt(selectedMonthsNumbers[i])-1;
        			HtmlSelectMonths += '<option value="'+month_number+'">';
        			
            		HtmlSelectMonths += ''+selectedMonths[i]+' '+year;
            		HtmlSelectMonths += '</option>';
        		}
        	}
        	HtmlSelectMonths += '</select>';
        		
            //console.log("show (month, year): ", month, " ", year)
            //lblTextMonth.text(months[month] + " " + year);
        	var lblTemp = $(HtmlSelectMonths);
        	lblTemp.change(function(){
        		value = $(this).val();
        		change_month(value, year);
        	});
        	//lblTextMonth.text(months[month] + " " + year);
        	//lblTextMonth.text(lblTemp);
        	lblTextMonth = lblTemp;
            //days of month
            var day_counter = 1;
        ...
        //calculation number of days in week
