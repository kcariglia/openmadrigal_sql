...
*
*     $Id: _Madrec.c 7607 2023-09-18 17:21:48Z brideout $
*/
...
static PyObject * _Madrec_getSummary(PyObject * self, PyObject * args)
{
   ...
   //create a python float to return min pulse length
    ...
    for (i=11; i<madrecp->numParms; i++)
    {
        ...
        /* verify parmArray not 0 */
        if (parmArray[i-11] == 0)
        {
        	sprintf(errString, "File %s is corrupt - contains illegal parameter id 0", filename);
        	PyErr_SetString(PyExc_IOError, errString);
        	return NULL;
        }
    }
...
static PyObject * _Madrec_getDerivableParms(PyObject * self, PyObject * args)
{
    ...
    {
    	pyTempValue = PyUnicode_FromString(dervParms->mnemList[i]);
    	PyList_Append(outputList, pyTempValue);
    ...
    //return all info;
    retObj = Py_BuildValue("O", outputList);
...
*   arguments:
*       1. Python string representing full path to file
*       2. Python string representing information string to print (non-functional for now)
...
static PyObject * _Madrec_getIsprintReport(PyObject * self, PyObject * args)
{
	...
	{
	    sprintf(errBuf, "Bad parm 2 %s in input Parm list in getIsprintReport - item %i", thisMnem, i);
	    PyErr_SetString(PyExc_TypeError, errBuf);
	    return NULL;
	}
	if (thisMnem != NULL)
...
static PyObject * _Madrec_looker_nonFile(PyObject * self, PyObject * args)
{
	...
	oneDValue = PyFloat_AsDouble(doubleObj);
	input1D[i] = oneDValue;
...
/***********************************************************************
*
* looker2_nonFile     prints a Madrecord for a random collection of lat, lon, and alt.
*                     Differs from looker in that allows the setting of any
*                     number of input 1D parameter2. Differs from looker_nonFile in
*                     that input positions are random, not a grid, and that 2D input
...
*       1. Python list of mnemonics of requested parameters
*          (Do not include gdlat, glon, or gdalt)
*       2. Python list of geodetic latitudes (float degrees -90 to 90)
*       3. Python list of longitudes (float degrees)
*       4. Python list of geodetic altiudes (float km) (all three lists must be same length)
*       5. ut (secs since 1/1/1950)
*       6. printHeaderFlag - if 0, do not print header
*       7. Python list of 1D parameters as mnemonic strings.  May be 0 length.
*           Cannot include gdalt, glon, or gdlat.
*       8. Python list of 1D parameter values as doubles.  Len must =
*           len of python list of 1D parameters
*       9. Python list of 2D parameters as mnemonic strings.  May be 0 length.
*           Cannot include gdalt, glon, or gdlat.
*      10. Python list of lists of 2D parameter values as doubles.  Len must =
*           len of python list of 2D parameters.  Each list must have length =
*           len of points and values = 2D parm value at each point in space
*
*   returns:
*       1. Python int 1
*
*   affects: madrecord for given collection of lat, lon, and alt printed.
*
*
*   throws:
*       PyExc_TypeError if illegal argument passed
*
*/
static PyObject * _Madrec_looker2_nonFile(PyObject * self, PyObject * args)
{
    PyObject * retObj;
    PyObject * pyItem;
    MadparmList * parmListShow;  /* list of parameters to show */
    ...
    PyObject * OneDParmList;
    PyObject * OneDParmValues;
    PyObject * TwoDParmList;
    PyObject * TwoDParmValues;
    PyObject * listObj;   /* python list object */
    PyObject * doubleObj; /* python double object */
    double oneDValue = 0.0;
    double twoDValue = 0.0;
    double posValue = 0.0;
    ...
    // get input arguments
    if (!PyArg_ParseTuple(args, "O!O!O!O!diO!O!O!O!",
			  &PyList_Type, &MnemList,
			  &PyList_Type, &LatList,
			  &PyList_Type, &LonList,
			  &PyList_Type, &AltList,
			  ...
			  &PyList_Type, &OneDParmList,
			  &PyList_Type, &OneDParmValues,
			  &PyList_Type, &TwoDParmList,
			  &PyList_Type, &TwoDParmValues))
    {
        return NULL;
    }
    ...
    if (numMnem == 0)
    {
        PyErr_SetString(PyExc_TypeError, "Must pass in at least one requested parameter to looker2_nonFile");
        return NULL;
    }

    /* check points lists are all same size */
	numPoints = PyList_Size(LatList);
	if (numPoints == 0)
	{
		PyErr_SetString(PyExc_TypeError, "Must pass in at least one position to looker2_nonFile");
		return NULL;
	}
	if (numPoints != PyList_Size(LonList) || numPoints != PyList_Size(AltList))
	{
		PyErr_SetString(PyExc_TypeError, "All position lists must be same size in looker2_nonFile");
		return NULL;
	}
    ...
    for (i=0; i<numMnem; i++)
    {
		...
		if (mnem == NULL)
		{
			sprintf(errBuf, "Illegal requested parameter at position %i in looker2_nonFile", i);
			PyErr_SetString(PyExc_TypeError, errBuf);
			return(NULL);
		}
		if (appendMadparm(parmListShow, mnem) == -1)
    ...
    /* get number of 1D and 2D parms */
    num1DParms = PyList_Size(OneDParmList);
    num2DParms = PyList_Size(TwoDParmList);

    /* verify right number of values passed in */
    if (PyList_Size(OneDParmValues) != num1DParms)
    {
		sprintf(errBuf, "number of 1D parms does not match number of 1D values in looker2_nonFile");
		PyErr_SetString(PyExc_TypeError, errBuf);
		return(NULL);
	}

    if (PyList_Size(TwoDParmValues) != num2DParms)
	{
		sprintf(errBuf, "number of 2D parms does not match number of 2D values in looker2_nonFile");
		PyErr_SetString(PyExc_TypeError, errBuf);
		return(NULL);
	}
    ...
    for (i=0; i<num1DParms; i++)
    {
		...
		if (mnem == NULL)
		{
			sprintf(errBuf, "Illegal 1D parameter at position %i in looker2_nonFile", i);
			PyErr_SetString(PyExc_TypeError, errBuf);
			return(NULL);
		}
		appendMadparm(parmMeas1D, mnem);
		...
		if (!str_caseins_cmp(mnem, "GDLAT") || \
			...
			!str_caseins_cmp(mnem, "GDALT"))
		{
			PyErr_SetString(PyExc_TypeError, "Cannot set GDALT, GLON, or GDLAT as 1D parm in looker2_nonFile");
			return NULL;
		}
    ...
    /* malloc input2D */
	input2D = (double **)malloc((3+num2DParms)*sizeof(double *));
	if (input2D == NULL)
	{
		PyErr_SetString(PyExc_TypeError, "Malloc");
		return(NULL);
	}
	for (i=0; i<(3+num2DParms); i++)
	{
		input2D[i] = (double *)malloc(numPoints * sizeof(double));
		if (input2D[i] == NULL)
		{
			PyErr_SetString(PyExc_TypeError, "Malloc");
			return(NULL);
	...
	{
		doubleObj = PyList_GetItem(LatList, i);
		posValue = PyFloat_AsDouble(doubleObj);
		input2D[0][i] = posValue;
	}

	/* write lon data next */
	for (i=0; i<numPoints; i++)
	{
		doubleObj = PyList_GetItem(LonList, i);
		posValue = PyFloat_AsDouble(doubleObj);
		input2D[1][i] = posValue;
		if (input2D[1][i] > 180.0)
			input2D[1][i] -= 360.0;
	}
	...
	for (i=0; i<numPoints; i++)
	{
		doubleObj = PyList_GetItem(AltList, i);
		posValue = PyFloat_AsDouble(doubleObj);
		input2D[2][i] = posValue;
	}

    /* create parmMeas2D and fill with data */
    parmMeas2D = createMadparmList();
    appendMadparm(parmMeas2D, "GDLAT");
    appendMadparm(parmMeas2D, "GLON");
    appendMadparm(parmMeas2D, "GDALT");
	...
	{
		pyItem = PyList_GetItem(TwoDParmList, i);
		mnem = getMnemFromPy(pyItem);
		if (mnem == NULL)
		{
			sprintf(errBuf, "Illegal 2D parameter at position %i in looker2_nonFile", i);
			PyErr_SetString(PyExc_TypeError, errBuf);
			return(NULL);
		}
		appendMadparm(parmMeas2D, mnem);
		...
		if (!str_caseins_cmp(mnem, "GDLAT") || \
			...
			!str_caseins_cmp(mnem, "GDALT"))
		{
			PyErr_SetString(PyExc_TypeError, "Cannot set GDALT, GLON, or GDLAT as 2D parm in looker2_nonFile");
			return NULL;
		}

		/* now set values */
		listObj = PyList_GetItem(TwoDParmValues, i);
		/* make sure the list is the right length */
		if (PyList_Size(listObj) != numPoints)
		{
			PyErr_SetString(PyExc_TypeError, "A TwoDValue list is not right length in looker2_nonFile");
			return NULL;
		}
		for (j=0; j<numPoints; j++)
		{
			doubleObj = PyList_GetItem(listObj, j);
			twoDValue = PyFloat_AsDouble(doubleObj);
			input2D[3+i][j] = twoDValue;
		}
	}
...
static PyObject * _Madrec_createMaddata(PyObject * self, PyObject * args)
{
    ...
    {
    	pyTempValue = PyLong_FromLong((long)maddata->madCycleList[i]->numMadrecords);
    	PyList_Append(outputList, pyTempValue);
    	Py_DECREF(pyTempValue);
    }
...
static PyObject * _Madrec_madrecDumpDataRecord(PyObject * self, PyObject * args)
{
    ...
    for (i=0; i<jpar; i++)
    {
        ...
        {
        	pyTempValue = PyLong_FromLong((long)oneDCodeList[i]);
        	PyList_Append(oneDParmList, pyTempValue);
        	Py_DECREF(pyTempValue);
        }
        free(desc);
    }
    ...
    for (i=0; i<mpar; i++)
    {
        ...
        if (strstr(desc, "dditional increment") == NULL)
        {
        	pyTempValue = PyLong_FromLong((long)twoDCodeList[i]);
        	PyList_Append(twoDParmList, pyTempValue);
        	Py_DECREF(pyTempValue);
        }
        free(desc);
    }
    ...
    for (i=0; i<PyList_Size(oneDParmList); i++)
    {
        ...
        if (isnan(data))
        {
        	pyTempValue = PyUnicode_FromString("missing");
            PyList_Append(oneDDataList, pyTempValue);
            Py_DECREF(pyTempValue);
        }
        else if (data == assumed && thisCode < 0)
        {
        	pyTempValue = PyUnicode_FromString("assumed");
        	PyList_Append(oneDDataList, pyTempValue);
        	Py_DECREF(pyTempValue);
        }
        else if (data == knownbad && thisCode < 0)
        {
        	pyTempValue = PyUnicode_FromString("knownbad");
        	PyList_Append(oneDDataList, pyTempValue);
        	Py_DECREF(pyTempValue);
        }
        /* handle real data */
        ...
        {
        	pyTempValue = PyFloat_FromDouble(data);
            PyList_Append(oneDDataList, pyTempValue);
            Py_DECREF(pyTempValue);
        }
    }
    ...
    for (j=0; j<nrow; j++)
    {
        for (i=0; i<PyList_Size(twoDParmList); i++)
        {
			...
			{
				pyTempValue = PyUnicode_FromString("missing");
				PyList_Append(twoDDataList, pyTempValue);
				Py_DECREF(pyTempValue);
			}
			else if (data == assumed && thisCode < 0)
			{
    ...
    Py_DECREF(oneDParmList);
    Py_DECREF(twoDParmList);
    Py_DECREF(oneDDataList);
    Py_DECREF(twoDDataList);
...
static PyObject * _Madrec_cedarSet2dParm(PyObject * self, PyObject * args)
{
	...
	if (PyUnicode_Check(PyItem))
	{
	    if (strcmp(PyUnicode_DATA(PyItem), "missing")==0)
                valueList[i] = missing;
	    else if (strcmp(PyUnicode_DATA(PyItem), "assumed")==0)
                valueList[i] = assumed;
	    else if (strcmp(PyUnicode_DATA(PyItem), "knownbad")==0)
                valueList[i] = knownbad;
            else
            {
                sprintf(errBuf, "Bad special parameter indicator <%s> in cedarSet2dParm, position %i",
                		(char *)PyUnicode_DATA(PyItem), i);
	        PyErr_SetString(PyExc_TypeError, errBuf);
		return(0);
            }
	}
...
/***********************************************************************
*
* madGetParIndexFromMnemonic     gets cedar parameter index from mnemonic string
*
* Used for ordering parameters according to parcods.tab
*
*   arguments:
*       Python string for cedar parameter mnemonic (case insensitive)
*
*   returns:
*       Python Float representing index.  Error parameters = index of non-error parm + 1/2
*       This allows ordering of error parms just after non-error
*
*   throws:
*       PyExc_TypeError if illegal argument passed
*
*/
static PyObject * _Madrec_madGetParIndexFromMnemonic(PyObject * self, PyObject * args)
{
    int index = 0;
    double fIndex = 0.0;
    char * mnem;
    ...
    if (index == missingData)
    	...
    	fIndex = (index*-1.0) + 0.5;
    else
    	fIndex = (double)index;

    return Py_BuildValue("d", fIndex);
...
static PyObject * _Madrec_cedarGetInformation(PyObject * self, PyObject * args)
{
    ...
    int recFound = 0;                // keeps track of whether record found
    ...
    if (recFound == 0)
	{
		/* malloc single character */
		infoStr = (char *)malloc(1);
	    infoStr[0] = '\0';
	}
...
static PyObject * _Madrec_cedarCatalogHeaderList(PyObject * self, PyObject * args)
{
    ...
    while(madrecGetNextRec(madrecp) == 0)
    {
        ...
        {
        	pyTempValue = PyLong_FromLong(recCount);
            PyList_Append(retCatalogList, pyTempValue);
            Py_DECREF(pyTempValue);
        }
        ...
        {
        	pyTempValue = PyLong_FromLong(recCount);
            PyList_Append(retHeaderList, pyTempValue);
            Py_DECREF(pyTempValue);
        }
    ...
    Py_DECREF(retCatalogList);
    Py_DECREF(retHeaderList);
    return(retObj);
}
...
/***********************************************************************
*
* convertGeodeticAACGM     converts geodetic <--> AACGM  coords
*
*   arguments:
...
*                  1:  convert geodetic to AACGM  coordinates
*                 -1:  convert AACGM coordinates to geodetic
*
*   returns: list containing
...
*
*/
static PyObject * _Madrec_convertGeodeticAACGM(PyObject * self, PyObject * args)
{
    ...
    // get arguments
	if (!PyArg_ParseTuple(args, "dddi", &orgLat, &orgLon, &alt, &direction))
	{
		return NULL;
	}
	...
	{
		sprintf(errString, "Invalid direction - must be 1 for GEOD->AACGM, or -1 for AACGM->GEOD, not %i", direction);
		PyErr_SetString(PyExc_TypeError, errString);
		return NULL;
	}
	...
	if (direction == 1)
	{
		...
		flag = 1;
		CONVRT_F77(&flag, &orgLat, &alt, &gclat, &rkm);

		CONVERT_GEO_COORD_F77(&gclat, &orgLon, &alt, &newLat, &newLon, &flag, &i_error);

		if (i_error != 0)
		{
			sprintf(errString, "convert_geo_coord failed");
			PyErr_SetString(PyExc_TypeError, errString);
			return NULL;
		}
	}
	...
	{
		flag = 2;
		CONVERT_GEO_COORD_F77(&orgLat, &orgLon, &alt, &gclat, &newLon, &flag, &i_error);
		...
		CONVRT_F77(&flag, &newLat, &alt, &gclat, &rkm);

	}

    retList = PyList_New(0);

    pyTempValue = PyFloat_FromDouble(newLat);
	PyList_Append(retList, pyTempValue);
	Py_DECREF(pyTempValue);

	pyTempValue = PyFloat_FromDouble(newLon);
	PyList_Append(retList, pyTempValue);
	Py_DECREF(pyTempValue);
...
static PyObject * _Madrec_pyTraceMagneticField(PyObject * self, PyObject * args)
{
    ...
    pyTempValue = PyFloat_FromDouble(out1);
    PyList_Append(retList, pyTempValue);
    Py_DECREF(pyTempValue);

    pyTempValue = PyFloat_FromDouble(out2);
    PyList_Append(retList, pyTempValue);
    Py_DECREF(pyTempValue);

    pyTempValue = PyFloat_FromDouble(out3);
    PyList_Append(retList, pyTempValue);
    Py_DECREF(pyTempValue);
...
*   			1. one way faraday rotation in radians, missing if error
...
*
*/
static PyObject * _Madrec_faradayRotation(PyObject * self, PyObject * args)
{
    ...
    // get arguments
    if (!PyArg_ParseTuple(args, "iiiiiiddddddd",
    		              &year,  &month, &day,
						  &hour, &min, &sec,
						  &sgdlat, &slon, &sgdalt,
						  &gdlat, &glon, &gdalt,
    ...
    result = faraday_rotation(year, month, day, hour, min, sec,
    		                  sgdlat, slon, sgdalt,
    		                  gdlat, glon, gdalt, freq,
    		                  &tec, &total_tec);
...
static void addIntListToPythonList(int * intArray, PyObject * pyList)
{
    ...
    {
    	pyTempValue = PyLong_FromLong(intArray[i]);
    	PyList_Append(pyList, pyTempValue);
    	Py_DECREF(pyTempValue);
    }
...
static void setPyTime(PyObject * retDateTime, double sec)
{
    ...
    pyTempValue = PyLong_FromLong(iyr);
    PyList_Append(retDateTime, pyTempValue);
    Py_DECREF(pyTempValue);
    ...
    pyTempValue = PyLong_FromLong(ics/100);
    PyList_Append(retDateTime, pyTempValue);
    Py_DECREF(pyTempValue);
}
...
static char * getMnemFromPy(PyObject * pyArg)
{
    ...
    else if (PyBytes_Check(pyArg))
    {
        	temp = PyBytes_AsString(pyArg);
    ...
    /* check if its a unicode 1byte string */
    else if (PyUnicode_Check(pyArg))
    {
        temp = PyUnicode_DATA(pyArg);
		thisMnem = (char *)malloc(sizeof(char)*(strlen(temp)+1));
		strcpy(thisMnem, temp);
		if (cedarGetParCodeFromMnemonic(thisMnem) == missingData)
		{
...
static PyMethodDef _MadrecMethods[] =
{
	  ...
	  {"looker2_nonFile",              _Madrec_looker2_nonFile,              METH_VARARGS},
	  ...
	  {"madGetParIndexFromMnemonic",   _Madrec_madGetParIndexFromMnemonic,  METH_VARARGS},
	  ...
	  {"convertGeodeticAACGM",         _Madrec_convertGeodeticAACGM,        METH_VARARGS},
	  {"faradayRotation",   	       _Madrec_faradayRotation,  			METH_VARARGS},
      {NULL,      NULL}        /* Sentinel */
};


static struct PyModuleDef _Madrec =
{
    PyModuleDef_HEAD_INIT,
    "_Madrec", /* name of module */
    "",          /* module documentation, may be NULL */
    -1,          /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    _MadrecMethods
};

PyMODINIT_FUNC PyInit__Madrec()
{
	import_array();
    return(PyModule_Create(&_Madrec));
}
