...
$Id: __bgReport_original.py 7043 2019-10-07 19:10:59Z brideout $
"""
import os
import os.path
import pickle
...
def __getFilterStrList(filterList):
    ...
    for filter in filterList:
        ...
        if tempList[1].strip() in ('*', '/', '+', '-'):
            ...
            for i in range(numFilters):
                try:
                    retList[4].append(float(tempList[1 + 2*i].strip()))
                except ValueError:
                    retList[4].append(None)
                try:
                    retList[5].append(float(tempList[2 + 2*i].strip()))
                except ValueError:
                    retList[5].append(None)
...
try:
    ...
    if not isOkay:
        raise ValueError('Warning: Madrigal temporary report directory %s over disk quota of %s GB' % (madDB.getDocDirPath() + '/' + tempDir,
                                                                                         str(maxTempReports)))
