...
$Id: madCalculator.py 7046 2019-10-07 19:57:14Z brideout $
"""
...
try:
    ...
    if startLat == None or isnan(startLat):
        raise ValueError('valid startLat argument required!\n%s' % (usage))
    elif startLat < -90 or startLat > 90:
        raise ValueError('startLat must be between -90 and 90: %f' % (startLat))

    if endLat == None or isnan(endLat):
        raise ValueError('valid endLat argument required!\n%s' % (usage))
    elif endLat < -90 or endLat > 90:
        raise ValueError('endLat must be between -90 and 90: %f' % (endLat))

    if stepLat == None or isnan(stepLat):
        raise ValueError('valid stepLat argument required!\n%s' % (usage))

    if startLong == None or isnan(startLong):
        raise ValueError('valid startLong argument required!\n%s' % (usage))
    elif startLong < -180 or startLong > 180:
        raise ValueError('startLong must be between -180 and 180: %f' % (startLong))

    if endLong == None or isnan(endLong):
        raise ValueError('valid endLong argument required!\n%s' % (usage))
    elif endLong < -180 or endLong > 180:
        raise ValueError('valid endLong must be between -180 and 180: %f' % (endLong))

    if stepLong == None or isnan(stepLong):
        raise ValueError('valid stepLong argument required!\n%s' % (usage))

    if startAlt == None or isnan(startAlt):
        raise ValueError('valid startAlt argument required!\n%s' % (usage))
    elif startAlt < 0.0:
        raise ValueError('valid startAlt must be positive: %f' % (startAlt))

    if endAlt == None or isnan(endAlt):
        raise ValueError('valid endAlt argument required!\n%s' % (usage))
    elif endAlt < 0.0:
        raise ValueError('valid endAlt must be positive: %f' % (endAlt))

    if stepAlt == None or isnan(stepAlt):
        raise ValueError('valid stepAlt argument required!\n%s' % (usage))
