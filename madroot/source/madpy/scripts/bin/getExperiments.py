...
Returns comma-delimited data, one line for each experiment, with the following fields:
    ...
    23. timestamp (UT) of st_mtime of experiment directory. Set to 0 for non-local (useful for import_madrigal - added between
        Madrigal 2.6 and Madrigal 3.0)
        
    24. experiment access (0 for public, 2 for archive) (useful for import_madrigal - added between
        Madrigal 2.6 and Madrigal 3.0)
...
try:
    ...
    for opt in optlist:
        if opt[0] == '--code':
            ...
            raise ValueError('Illegal option %s' % (opt[0]))
    ...
    while 1:
        ...
        print(('%i,%s,%s,%i,%s,%i,%s,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%s,%s,%i,%i') % \
                (thisId,
                thisUrl,
                thisName,
                thisSiteId,
